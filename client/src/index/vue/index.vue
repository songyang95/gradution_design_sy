<template>
    <div class="wapper">
        ...
        <div class="search">
            <input type="text" v-model="searchKey" placeholder="按照名字检索">
        ...
        <div class="list" v-for="(item,index) in dataList">
            <div class="list_no">编号:{{index+1}}</div>
            <div class="list_name">姓名:{{item.name}}</div>
...
<script>
    ...
    export default {
        ...
        methods:{
            search(){
                window.location.href="/search?searchKey="+ this.searchKey;
            },
            deleted(id){
                var _this=this;
                axios.get("/deleted?id="+id)
                    .then(function (res) {
                        if(res.data == "success"){
                            alert("删除成功");
